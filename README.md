...
模型清单是一个数组，兼容所有 OpenAI 协议的服务。设置页**选提供商 → 贴 API Key →
下拉选模型**即可（内置智谱 / DeepSeek / MiniMax / Ollama / llama.cpp 预设，
「自定义」才手填端点）；模型下拉的候选从服务的 `/models` 实时拉取，可点「刷新」。**列表顺序就是优先级**：结果区按这个顺序排列，
...
- `tts_voice` / `tts_rate`：朗读音色与语速。空音色跟随系统默认（念中文常听着
  不对，推荐 `Tingting` 等 zh_CN 音色）；语速在设置页是 ±100 的滑杆（0 居中 = 默认，配置里存 0 或 175±偏移）。
